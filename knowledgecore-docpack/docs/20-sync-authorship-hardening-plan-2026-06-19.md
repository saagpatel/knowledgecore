...
9. Done: add a read-only `sync auth-strict-check` rollout gate and strict pull behavior; both fail with `KC_SYNC_AUTH_STRICT_BLOCKED` when a target/head is not strict-ready.
10. Done: make pull entrypoints default to strict sync auth while retaining explicit compatibility pull through CLI `--allow-legacy-auth` and RPC `strict_auth=false`.
11. Remaining: separately approve remote trust import and the point at which undeclared legacy BLAKE3-derived author signatures become hard failures.
...
- Added read-only `sync auth-strict-check` as an operator/CI rollout gate, made pull entrypoints strict by default, and retained explicit compatibility pull before any fallback removal.
